...
rm(list=ls())
setwd("C:/Users/Пользователь/Desktop/MA1y/Prediction_with_ML/Machine-Learning/Assignment 1")
...
cat("### Model 1: Basic Regression\n")
cat("$$ \\ln(\\text{Hourly Wage}) = \\beta_0 + \\beta_1 \\cdot \\text{race_black} + \\beta_2 \\cdot \\text{race_asian} + \\beta_3 \\cdot \\text{race_other} + \\beta_4 \\cdot \\text{female} + \\varepsilon $$\n\n")

cat("### Model 2: Adding Age Variables\n")
cat("$$ \\ln(\\text{Hourly Wage}) = \\beta_0 + \\beta_1 \\cdot \\text{race_black} + \\beta_2 \\cdot \\text{race_asian} + \\beta_3 \\cdot \\text{race_other} + \\beta_4 \\cdot \\text{female} + \\beta_5 \\cdot \\text{age} + \\beta_6 \\cdot \\text{age}^2 + \\varepsilon $$\n\n")

cat("### Model 3: Adding Marital Status\n")
cat("$$ \\ln(\\text{Hourly Wage}) = \\beta_0 + \\beta_1 \\cdot \\text{race_black} + \\beta_2 \\cdot \\text{race_asian} + \\beta_3 \\cdot \\text{race_other} + \\beta_4 \\cdot \\text{female} + \\beta_5 \\cdot \\text{age} + \\beta_6 \\cdot \\text{age}^2 + \\beta_7 \\cdot \\text{marital_married} + \\beta_8 \\cdot \\text{marital_nonmarried} + \\varepsilon $$\n\n")

cat("### Model 4: Full Model (Including Education)\n")
cat("$$ \\ln(\\text{Hourly Wage}) = \\beta_0 + \\beta_1 \\cdot \\text{race_black} + \\beta_2 \\cdot \\text{race_asian} + \\beta_3 \\cdot \\text{race_other} + \\beta_4 \\cdot \\text{female} + \\beta_5 \\cdot \\text{age} + \\beta_6 \\cdot \\text{age}^2 + \\beta_7 \\cdot \\text{marital_married} + \\beta_8 \\cdot \\text{marital_nonmarried} + \\beta_9 \\cdot \\text{educ_hsgrad} + \\beta_{10} \\cdot \\text{educ_college} + \\beta_{11} \\cdot \\text{educ_ba_plus} + \\varepsilon $$\n\n")
...
msummary(list(reg1, reg2, reg3, reg4),
         fmt="%.3f",
         gof_omit = 'DF|Deviance|Log.Lik.|F|AIC|R2',
         stars=TRUE,
         coef_rename = cm,
